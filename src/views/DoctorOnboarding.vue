<template>
  <div style="height:700px;width:100%">

    <p>
      <center><strong>Please enter in proper doctor login information</strong></center>
    </p>

    <center>
      <p>New Username:</p> <input type="text" name="username" placeholder="username" required>
      <br /><br />
      <p>New Password:</p> <input type="password" name="password" placeholder="password" required>

      <br /><br />
      <p>First Name:</p> <input type="text" name="fname" placeholder="enter firstname" required>

      <br /><br />
      <p>Last Name:</p> <input type="text" name="lname" placeholder="last name" required>

      <br /><br />
      <p> Social Security Number:</p> <input type="text" name="SSN" placeholder="enter SSN" required>

      <br /><br />
      <p>Date of Birth:</p> <input type="text" name="DOB" placeholder="month/date/year" required>
      <br /><br />
      <p>Sex:</p> <input type="text" name="sex" placeholder="male, female or neither" required>

      <br /><br />
      <p>Medical License Number:</p> <input type="text" name="lnumber" placeholder="********" required>


      <br /><br />
      <a class="waves-effect waves-light btn modal-trigger" href="#hippa2">Save Details</a>
    </center>

  </div>
</template>